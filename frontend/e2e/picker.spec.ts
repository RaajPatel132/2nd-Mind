...
type Turn = { models: Record<string, { model: string }>; usage: { input_tokens: number; cached_input_tokens: number; output_tokens: number; charged_tokens: number } }
...
test('picking a model runs the turn on it and charges the quota at its weight', async ({ page }, testInfo) => {
  ...
  expect(new Set(Object.values(turn.models).map((m) => m.model))).toEqual(new Set(['claude-opus-5']))
  const raw = turn.usage.input_tokens + turn.usage.cached_input_tokens + turn.usage.output_tokens
  // Each call rounds on its own, so the total is within a token per call of 2.5 × raw.
  expect(Math.abs(turn.usage.charged_tokens - raw * 2.5)).toBeLessThanOrEqual(Object.keys(turn.models).length)
